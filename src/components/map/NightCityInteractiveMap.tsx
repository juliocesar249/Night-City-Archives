...
export function NightCityInteractiveMap({
  ...
  const districtHotspots: Record<string, { x: string, y: string, width: string, height: string, name: string }> = {
    ...
    "as-terras-baldias-badlands-o-deserto-sem-lei": { x: "650", y: "50", width: "200", height: "600", name: "Badlands" }, // This is a large area, might need multiple rects or a polygon if your SVG is detailed
  };
  ...
  return (
    <div className="w-full max-w-3xl mx-auto my-6 p-2 border border-border rounded-lg shadow-lg bg-card overflow-hidden" aria-label="Mapa Interativo de Night City">
      ...
      >
        ...
        {/* YOUR SINGLE COMPLEX SVG PATH DATA GOES HERE 
            This path draws your base map. It won't be interactive itself.
            Example: <path d="M10 10 H 90 V 90 H 10 Z" fill="hsl(var(--muted))" stroke="hsl(var(--border))" strokeWidth="0.5" /> 
            Replace the 'd' attribute with your actual path data.
        */}
        <path
          ...
          strokeWidth="0.5"
          className="pointer-events-none" // Ensures this base map doesn't interfere with hotspot events
        />
        ...
        <g>
          {districts.map((district) => {
            ...
            if (!hotspot) {
              ...
              return null; // Skip rendering if no hotspot is defined
            }

            const isActive = activeDistrictId === districtId;
            const isHovered = hoveredDistrictId === districtId;
            
            let fill = "transparent"; // Default: invisible
            let stroke = "transparent"; // Default: invisible
            let strokeWidth = 0.5; // Default stroke width, won't show with transparent stroke

            if (isActive) {
              fill = `hsl(var(--primary) / 0.4)`; // More opaque primary color for active
              stroke = `hsl(var(--primary))`;
              strokeWidth = 2;
            } else if (isHovered) {
              fill = `hsl(var(--primary) / 0.25)`; // Semi-transparent primary color for hover
              stroke = `hsl(var(--primary))`;
              strokeWidth = 1.5;
            }
            ...
            return (
              <g key={districtId}>
                <rect
                  id={districtId} // ID for potential direct targeting if needed, though not used by current styling
                  x={hotspot.x}
                  y={hotspot.y}
                  width={hotspot.width}
                  ...
                  style={{ zIndex }} // zIndex might not have a direct effect in SVG stacking order this way, order of elements matters more.
                  aria-label={district.name}
                />
                {/* Optional: Text label for helping position hotspots - can be removed later */}
                {/* This text will also be transparent by default unless styled */}
                 { (isHovered || isActive) && (
                    <text 
                      x={parseFloat(hotspot.x) + parseFloat(hotspot.width) / 2} 
                      y={parseFloat(hotspot.y) + parseFloat(hotspot.height) / 2 + 5} // Adjust y for better centering
                      textAnchor="middle" 
                      dominantBaseline="middle"
                      fontSize="10" // Adjust as needed
                      fontWeight="bold"
                      fill="hsl(var(--primary-foreground))" // Text color on highlight
